...
const STATUSES=["Interested","Started","Applied","Assessment","Interview","Final Round","Offer","Rejected","Withdrawn"];
...
export default async function handler(req,res){
  res.setHeader("Access-Control-Allow-Origin","*");res.setHeader("Access-Control-Allow-Headers","Content-Type, X-Tracker-Key");res.setHeader("Access-Control-Allow-Methods","POST, OPTIONS");
  ...
  if(!id||!STATUSES.includes(status))return res.status(400).json({error:"Valid id and status are required"});
  ...
  const file=await fetch(`${API}/repos/${REPO}/contents/${FILE}`,{headers});if(!file.ok)return res.status(502).json({error:"Could not read tracker"});
  const meta=await file.json(),raw=Buffer.from(meta.content.replace(/\n/g,""),"base64").toString("utf8"),rows=parseCSV(raw),rec=rows.find(r=>r.id===id);
  if(!rec)return res.status(404).json({error:"Application not found"});
  const old=rec.status,today=new Date().toISOString().slice(0,10);rec.status=status;rec.last_updated=today;
  if(status==="Started"&&!rec.date_started)rec.date_started=today;if(status==="Applied"&&!rec.date_applied)rec.date_applied=today;
  ...
  const put=await fetch(`${API}/repos/${REPO}/contents/${FILE}`,{method:"PUT",headers:{...headers,"Content-Type":"application/json"},body:JSON.stringify({message:`Update ${id} status: ${old} → ${status}`,content:Buffer.from(out).toString("base64"),sha:meta.sha})});
  if(!put.ok)return res.status(502).json({error:"GitHub update failed",detail:(await put.text()).slice(0,300)});
